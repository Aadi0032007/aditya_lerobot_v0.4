# -*- coding: utf-8 -*-
"""
Created on Sat Feb 28 05:18:39 2026
...
class RevobotsHdiFollowerInit(Robot):
    ...
    def ava_left_arm_offset(self, index: int, value_deg: float) -> int:
        if index == 0:
            ...
            return int((int(value_deg) - 75) * 3600 * -1)
        elif index == 5:
            return int((50 - int(value_deg)) * 71.1111)
        elif index == 6:
            return int(2200 + (50 - value_deg) * 25.778)
        else:
            return int(value_deg * 3600)
    ...
    def send_action(self, action: dict[str, float]) -> dict[str, float]:
        ...
        for i, value in enumerate(values_list):
            ...
            if self.config.socket_ip == "100.87.226.50":
    ...
    def write_init(self):
        ...
        init_config_lst = [
                ...
                "S, J4BoundryHigh, 420000",
                "S, J4BoundryLow, -420000",
...
def main():
    print("starting left arm initialisation")
    robot_config = RevobotsHdiFollowerConfig(
        id="my_revobots_hdi_follower",
        socket_ip="100.79.109.42",     # use your robot IP if it’s network-based
    )
    print("connecting")
    ...
    print("starting right arm initialisation")
    robot_config = RevobotsHdiFollowerConfig(
        id="my_revobots_hdi_follower",
        socket_ip="100.87.226.50",     # use your robot IP if it’s network-based
    )
    print("connecting")
    robot = RevobotsHdiFollowerInit(robot_config)
    robot.connect()
