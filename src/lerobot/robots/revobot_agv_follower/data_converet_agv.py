...
def batch_convert_sessions(
    ...
    for episode_idx, json_path in enumerate(json_files):
        ...
        for i, row in enumerate(log_data):
            ...
            if not ret:
                logger.warning(f"Video ended prematurely at frame {i}. Stopping extraction.")
                break

            # Convert OpenCV BGR to RGB
            frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
            
            # Extract values using fallbacks: try the long name first, then the short name
            # Default to 0.0 just in case neither is found to prevent crashes
            lin_val = row.get("linear_x", row.get("lin_x", 0.0))
            ang_val = row.get("angular_z", row.get("ang_z", 0.0))
            
            # A. Reconstruct RAW Observation
            raw_obs = {
                "front": frame_rgb,
                "lin_x": lin_val,
                "ang_z": ang_val
            ...
            # B. Reconstruct RAW Action
            raw_act = {
                "lin_x": lin_val,
                "ang_z": ang_val
            }

            # C. Mimic LeRobot's internal formatting exactly
            ...
            # D. Combine and add to the dataset buffer
