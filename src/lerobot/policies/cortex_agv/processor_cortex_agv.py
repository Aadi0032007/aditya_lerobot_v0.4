...
def make_cortex_agv_pre_post_processors(
    ...
    """Creates the pre- and post-processing pipelines for the Cortex AGV policy.
    ...
    Note: when `config.discretize_actions` is enabled, the policy's bin centers live in the same normalized
    action space produced by this pre-processor, so the same normalization stats must be used for both.

    Args:
        config (CortexAGVConfig): The Cortex AGV policy configuration object.
